<!-- Optional JavaScript -->
<script src="https://cdn.jsdelivr.net/npm/@popperjs/core@2.6.0/dist/umd/popper.min.js" integrity="sha384-KsvD1yqQ1/1+IA7gi3P0tyJcT3vR+NdBTt13hSJ2lnve8agRGXTTyNaBYmCR/Nwi" crossorigin="anonymous"></script>
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.0.0-beta2/dist/js/bootstrap.min.js" integrity="sha384-nsg8ua9HAw1y0W1btsyWgBklPnCUAFLuTMS2G72MMONqmOymq585AcH49TLBQObG" crossorigin="anonymous"></script>
<!-- <script src="http://localhost/beatmysquad/assets/plugins/owlCarousel/assets/owlcarousel/owl.carousel.min.js"></script> -->

<?php if (SCRIPT_ROOT == $actual_link) { ?>
    <script src="<?php echo SCRIPT_ROOT ?>assets/plugins/owlCarousel/assets/owlcarousel/owl.carousel.min.js"></script>

<?php } else if (SCRIPT_ROOT.'index.php' == $actual_link) { ?>
    <script src="<?php echo SCRIPT_ROOT ?>assets/plugins/owlCarousel/assets/owlcarousel/owl.carousel.min.js"></script>

<?php } 
else if (SCRIPT_ROOT . 'page_name.php' == $actual_link) { ?>



<?php } else if (SCRIPT_ROOT . 'page_name2.php' == $actual_link) { ?>



<?php } else if (SCRIPT_ROOT . 'page_name3.php' == $actual_link) { ?>



<?php } else if (SCRIPT_ROOT . 'page_name4.php' == $actual_link) { ?>



<?php } ?>


<script src="<?php echo SCRIPT_ROOT ?>assets/js/custom.js"></script>
<script src="<?php echo SCRIPT_ROOT ?>assets/plugins/owlCarousel/assets/owlcarousel/owl.carousel.min.js"></script>





</body>

</html>